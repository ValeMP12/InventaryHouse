...
function remove_matching(arr, obj) {
...
function get_backend_name(target) {
...
function is_socket_request(req) {
...
function get_headers(req) {
  ...
  if (req.connection || req.socket) {
    ...
    for (var key in connection_data) {
      ...
      if (headers[name])
        headers[name] += ', ' + connection_data[key];
      else
        headers[name] = connection_data[key];
    }
  }
...
exports.send = function(req, res) {
  ...
  req.connection.setNoDelay(true); // disable nagle algorythm
  req.connection.setTimeout(0);    // we'll set or own timer

  var request_opts     = target;
  request_opts.agent   = agent;
  request_opts.method  = req.method;
  request_opts.path    = req.url;
  request_opts.headers = get_headers(req);
  ...
  function proxy_error(err) {
    error = true;
    if (timer) clearTimeout(timer);
    log('Proxy request error [' + err.code + ']', err.message);

    if (!res.headersSent) {
      if (opts.retry_failed === true) {
        // retry is enabled, so wait one second and resend the request
        return setTimeout(function() {
          log('Retrying request: ' + req.method + ' ' + req.url);
    ...
    if (req.method !== 'HEAD')
      res.end(messages[res.statusCode] + '\n');
  ...
  var proxy_request = http.request(request_opts);

  // Ensure we abort proxy if request is aborted
  req.once('aborted', function() {
    // if proxy request hasn't timeout out, then it means that the original 
    // request from the client was aborted.  
    if (!req.timed_out)
      log('Request aborted by client!');
  ...
  */
  ...
  proxy_request.on('response', function(proxy_response) {
    ...
    var headers = proxy_response.headers; // node returns them in lowercase form
    if (backend_name)
    ...
    res.statusCode = proxy_response.statusCode;
    for (var h in headers) {
      res.setHeader(header_func(h), headers[h]);
    }

    /*
    emit('proxy_response', proxy_response);

    // Allow us to listen when the proxy has completed
    proxyRes.on('end', function () {
      emit('end', req, res, proxy_response);
    })
    */
  ...
  // req.on('error', proxy_error); // this should be handled by the parent server
  ...
  timer = setTimeout(function() {
    ...
    if (!res.writable)
      return done(new Error('No response from backend in ' + timeout/1000 + ' seconds.'));
