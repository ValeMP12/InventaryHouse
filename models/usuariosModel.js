...
const createUsuario = async (nombre, cargo, horario, turno, telefono) => {
    try {
        const query = 'INSERT INTO usuario (nombre, cargo, horario, turno, telefono) VALUES ($1, $2, $3, $4, $5) RETURNING *';
        const values = [nombre, cargo, horario, turno, telefono];
        const result = await pool.query(query, values);
        return result.rows[0];
    } catch (error) {
        throw error;
...
const updateUsuario = async (id_usuario, nombre, cargo, horario, turno, telefono) => {
    try {
        ...
        const values = [nombre, cargo, horario, turno, telefono, id_usuario];
        const result = await pool.query(query, values);
        return result.rows[0];
    } catch (error) {
        throw error;
...
const getCargoByIdUsuario = async (id) => {
    try {
        const query = 'SELECT d.nombre, e.id_usuario FROM cargo e INNER JOIN cargo d ON d.id_usuario = e.id_usuario WHERE e.id_usuario = $1';
        const values = [id];
        const result = await pool.query(query, values);
        return result.rows[0];
    } catch (error){
